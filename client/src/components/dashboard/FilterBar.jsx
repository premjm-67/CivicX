export default function FilterBar({ filters, onChange }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 flex flex-wrap gap-3">
      ...
      <input type="text" placeholder="Search zone..." value={filters.zone || ''} onChange={e => set('zone', e.target.value)}
        className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-40" />
      <input type="text" placeholder="Search street..." value={filters.street || ''} onChange={e => set('street', e.target.value)}
        className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-40" />
      ...
      <button onClick={() => onChange({ area: '', zone: '', street: '', status: '', priority: '', category: '' })}
